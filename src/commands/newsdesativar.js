module.exports = {
    name: 'newsdesativar',
    aliases: ['desativarnews'],
    category: 'grupos',
    description: 'Desativa o serviço de feed de notícias (global)',
    async execute(sock, m, { from, isGroup, sender, config, utils, lastBotResponse, GLOBAL_COOLDOWN }) {
        ...
        if (svc) {
            try { svc.stop(); } catch (e) { console.error('[newsdesativar] stop falhou:', e?.message || e); }
        }
