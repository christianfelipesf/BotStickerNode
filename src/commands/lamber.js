...
module.exports = {
    ...
    async execute(sock, m, { from, sender, utils, lastBotResponse, GLOBAL_COOLDOWN }) {
        ...
        if (!targetJid) { await sock.sendMessage(from, { text: '❌ Marque alguém: `!lamber @user` ou responda a mensagem.' }, { quoted: m }); return cur; }
        const caption = `👅 *@${sender.split('@')[0]}* lambeu *@${targetJid.split('@')[0]}*`;
        const mentions = [sender, targetJid];
        let buf = null; try { buf = await fetchInteractionImage('lamber'); } catch (e) { console.error('❌ [lamber] fetch falhou:', e.message); }
