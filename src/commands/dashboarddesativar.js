module.exports = {
    name: 'dashboarddesativar',
    aliases: ['dashdesativar'],
    category: 'grupos',
    description: 'Desativa o dashboard (global)',
    async execute(sock, m, { from, isGroup, sender, config, utils, lastBotResponse, GLOBAL_COOLDOWN }) {
        ...
        if (svc) {
            try {
                await svc.stop();
                console.log('[dashboard] desativado via comando.');
            } catch (e) { console.error('[dashboarddesativar] stop falhou:', e?.message || e); }
        }
