...
function init(config) {
    // dashboardEnabled: true (default) | false
    // dashboardPort:    3000 (default) — porta do servidor HTTP/WebSocket
...
<body class="dark">
    ...
    <div id="header">
        ...
        <div class="theme-selector">
            <button id="notif-btn" class="theme-btn" style="background: var(--wa-green-light); color: white;" onclick="toggleSound()">SOM: OFF</button>
            <button id="push-btn" class="theme-btn" style="background: #3498db; color: white;" onclick="togglePush()">NOTIF: OFF</button>
    ...
    <script>
        ...
        function toggleSound() {
            ...
            notifBtn.innerText = \`SOM: \${soundEnabled ? 'ON' : 'OFF'}\`;
            ...
            if (!pushEnabled) {
                ...
                Notification.requestPermission().then(permission => {
                    if (permission === "granted") {
                        pushEnabled = true;
                        pushBtn.innerText = "NOTIF: ON";
                        pushBtn.style.background = "#2980b9";
                    } else {
                ...
                pushEnabled = false;
                pushBtn.innerText = "NOTIF: OFF";
                pushBtn.style.background = "#3498db";
            }
        }
            ...
            if (isNew) {
                ...
                if (pushEnabled && document.visibilityState !== 'visible') {
                    ...
                    if (data.type === 'chat') {
                        ...
                        body = data.text || (data.media ? 'Mídia recebida' : 'Nova mensagem');
                        iconName = data.name || 'User';
                        iconBg = '25d366';
                        tag = 'chat-' + (data.phone || 'msg');
                    } else if (data.type === 'action') {
                        title = '⚙️ Ação do bot' + (data.group ? ' · ' + data.group : '');
                        ...
                        body = data.text || 'Ocorreu um erro no bot';
                        iconName = '!';
                        iconBg = 'e74c3c';
                        tag = 'error';
                    } else {
                    ...
                    try {
                        new Notification(title, {
                            body,
                            icon: 'https://ui-avatars.com/api/?name=' + encodeURIComponent(iconName) + '&background=' + iconBg + '&color=fff',
                            tag,
                            renotify: true
                        });
                    } catch (e) {}
