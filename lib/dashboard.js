...
function init(config) {
    ...
    const server = http.createServer(async (req, res) => {
        ...
        if (req.method === 'POST' && reqPath === '/api/reply') {
        ...
        if (req.method === 'POST' && reqPath === '/api/send') {
            await readJsonBody(req, res, async (payload) => {
                try {
        ...
        if (req.method === 'GET' && reqPath === '/api/health') {
            sendJson(res, 200, { ok: !!sockRef });
            return;
        }

        if (reqPath.startsWith('/api/')) {
            sendJson(res, 404, { ok: false, error: `Endpoint nao encontrado: ${reqPath}` });
            return;
        }
...
function readJsonBody(req, res, cb) {
    ...
    req.on('data', c => {
        ...
        if (body.length > MAX_JSON_BODY) {
            tooLarge = true;
            body = '';
            sendJson(res, 413, { ok: false, error: 'Arquivo muito grande para enviar pelo dashboard' });
        }
    });
    req.on('end', () => {
        if (tooLarge) return;
    ...
    req.on('error', () => {
        if (!res.headersSent) sendJson(res, 400, { ok: false, error: 'Falha ao receber dados do envio' });
    });
}
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
        ...
        .msg.sent { align-self: flex-end; border-top-right-radius: 0; background-color: var(--msg-out-light); }
        body.dark .msg.sent, body.oled .msg.sent { background-color: var(--msg-out-dark); color: var(--text-primary-dark); }
