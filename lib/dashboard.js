...
const GROUP_INFO_TTL = 10 * 60 * 1000;
...
async function getGroupInfo(item) {
    const base = normalizeGroupItem(item);
    if (!base) return null;

    const cached = groupInfoCache.get(base.jid);
    if (cached && Date.now() - cached.updatedAt < GROUP_INFO_TTL) {
        return {
            ...base,
            ...cached.info,
...
async function getGroupsSnapshot() {
    ...
    for (const item of items) {
        ...
        seen.add(base.jid);
        const info = await getGroupInfo(base);
...
function init(config) {
    ...
    app.get('/favicon.ico', (req, res) => {
        const ico = path.join(__dirname, '..', 'favcon.jpg');
...
async function pushGroupsSnapshot() {
    if (!ioServer) return;
    try {
        const list = await getGroupsSnapshot();
...
module.exports = {
    ...
    setGroupsApi, pushGroupsSnapshot
};
