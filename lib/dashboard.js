        ...
        function appendMessage(data, isNew = true) {
            ...
            if (isNew) {
                ...
                if (pushEnabled && document.visibilityState !== 'visible') {
                    const title = data.name ? `${data.name} (@${data.phone})` : data.group;
                    const options = {
                        body: data.text || 'Mídia recebida',
                        icon: 'https://ui-avatars.com/api/?name=' + (data.name || 'Bot').replace(/\s/g, '+') + '&background=25d366&color=fff'
                    };
